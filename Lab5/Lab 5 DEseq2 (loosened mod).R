...
#WARNING: Normally should use $padj. Modified for class assignment only

#Show how many genes pavalue < 0.05
...
res$sig <- ifelse(res$pval < 0.05 & res$log2FC >= 0 & !is.na(res$pval) | res$pval < 0.05 & res$log2FC <= -1 & !is.na(res$pval) , "DE", "notDE")
head(res)
table(res$sig)
#Make the plot
ggplot(res) + geom_point(aes(x=log2(baseMeanA),y=log2(baseMeanB),color=sig))

write.csv(res, "/Users/maxharman/Library/CloudStorage/OneDrive-MichiganStateUniversity/Documents/Michigan State/PLB812/PLB812-main/Lab5\\Lab6_DESeq_output.csv")
